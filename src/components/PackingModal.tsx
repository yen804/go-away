...
import { X, CheckCircle2, Plus, Trash2, Package } from 'lucide-react';
...
const PackingModal = () => {
  ...
  const [items, setItems] = useState<PackingItem[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: '1', name: '護照/簽證', category: '必備證件', checked: false },
      { id: '2', name: '日幣現金', category: '必備證件', checked: false },
      { id: '3', name: '行動電源', category: '電子產品', checked: false }
    ];
  });
  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="w-full bg-[#A8E6CF] border-4 border-black p-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex items-center justify-between font-black text-xl hover:translate-x-1 hover:translate-y-1 transition-all"
      >
        <span>行李檢查清單</span>
        <Package size={24} />
      </button>
        ...
        <div className="p-6 space-y-6">
          <div className="bg-[#F7F7F7] border-4 border-black p-4 space-y-3">
            <input 
              ...
              className="w-full border-4 border-black p-2 font-bold outline-none"
            />
            <div className="flex gap-2">
              <select 
                value={selectedCat}
                onChange={(e) => setSelectedCat(e.target.value)}
                className="flex-1 border-4 border-black p-2 font-bold outline-none bg-white"
              >
                {categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
              <button onClick={addItem} className="bg-black text-white px-4 py-2 font-black">
                ADD
              </button>
            </div>
          </div>
          ...
          {categories.map(cat => {
            ...
            return (
              <div key={cat} className="space-y-2">
                <h3 className="font-black text-sm bg-[#FFD93D] inline-block px-2 py-1 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  {cat}
                </h3>
                <div className="space-y-1">
                  {catItems.map(item => (
                    <div 
                      key={item.id} 
                      className={`flex items-center justify-between p-3 border-2 border-black transition-colors ${item.checked ? 'bg-gray-100 opacity-50' : 'bg-white'}`}
                    >
